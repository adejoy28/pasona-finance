...
import { SearchParamsExtractor } from './SearchParamsExtractor';
...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = errResponse.response?.data?.email?.[0] ?? errResponse.response?.data?.message ?? 'Failed to reset password. Please try again.';
      setError(message);
    } finally {
      setLoading(false);
    }
